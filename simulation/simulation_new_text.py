...
class YardSimApp:
    ...
    def _build_ui(self):
        ...
        self.bucket_canvas_frame = ttk.Frame(self.right)
        self.bucket_canvas_frame.grid(row=0, column=0, sticky="nsew", padx=5, pady=5)
        self.bucket_canvas_frame.grid_rowconfigure(0, weight=1)
        self.bucket_canvas_frame.grid_columnconfigure(0, weight=1)

        self.bucket_canvas = tk.Canvas(self.bucket_canvas_frame, bg="white", width=720, height=620)
        self.bucket_canvas.grid(row=0, column=0, sticky="nsew")

        self.bucket_canvas_vsb = ttk.Scrollbar(self.bucket_canvas_frame, orient=tk.VERTICAL, command=self.bucket_canvas.yview)
        self.bucket_canvas_vsb.grid(row=0, column=1, sticky="ns")
        self.bucket_canvas_hsb = ttk.Scrollbar(self.bucket_canvas_frame, orient=tk.HORIZONTAL, command=self.bucket_canvas.xview)
        self.bucket_canvas_hsb.grid(row=1, column=0, sticky="ew")
        self.bucket_canvas.configure(yscrollcommand=self.bucket_canvas_vsb.set, xscrollcommand=self.bucket_canvas_hsb.set)

        self.baseline_canvas_frame = ttk.Frame(self.right)
        self.baseline_canvas_frame.grid(row=0, column=1, sticky="nsew", padx=5, pady=5)
        self.baseline_canvas_frame.grid_rowconfigure(0, weight=1)
        self.baseline_canvas_frame.grid_columnconfigure(0, weight=1)

        self.baseline_canvas = tk.Canvas(self.baseline_canvas_frame, bg="white", width=720, height=620)
        self.baseline_canvas.grid(row=0, column=0, sticky="nsew")

        self.baseline_canvas_vsb = ttk.Scrollbar(self.baseline_canvas_frame, orient=tk.VERTICAL, command=self.baseline_canvas.yview)
        self.baseline_canvas_vsb.grid(row=0, column=1, sticky="ns")
        self.baseline_canvas_hsb = ttk.Scrollbar(self.baseline_canvas_frame, orient=tk.HORIZONTAL, command=self.baseline_canvas.xview)
        self.baseline_canvas_hsb.grid(row=1, column=0, sticky="ew")
        self.baseline_canvas.configure(yscrollcommand=self.baseline_canvas_vsb.set, xscrollcommand=self.baseline_canvas_hsb.set)

        self.bucket_text_frame = ttk.Frame(self.right)
        self.bucket_text_frame.grid(row=1, column=0, sticky="nsew", padx=5, pady=5)
        self.bucket_text_frame.grid_rowconfigure(0, weight=1)
        self.bucket_text_frame.grid_columnconfigure(0, weight=1)

        self.bucket_text = tk.Text(self.bucket_text_frame, height=11, width=85)
        self.bucket_text.grid(row=0, column=0, sticky="nsew")
        self.bucket_text_vsb = ttk.Scrollbar(self.bucket_text_frame, orient=tk.VERTICAL, command=self.bucket_text.yview)
        self.bucket_text_vsb.grid(row=0, column=1, sticky="ns")
        self.bucket_text.configure(yscrollcommand=self.bucket_text_vsb.set)

        self.baseline_text_frame = ttk.Frame(self.right)
        self.baseline_text_frame.grid(row=1, column=1, sticky="nsew", padx=5, pady=5)
        self.baseline_text_frame.grid_rowconfigure(0, weight=1)
        self.baseline_text_frame.grid_columnconfigure(0, weight=1)

        self.baseline_text = tk.Text(self.baseline_text_frame, height=11, width=85)
        self.baseline_text.grid(row=0, column=0, sticky="nsew")
        self.baseline_text_vsb = ttk.Scrollbar(self.baseline_text_frame, orient=tk.VERTICAL, command=self.baseline_text.yview)
        self.baseline_text_vsb.grid(row=0, column=1, sticky="ns")
        self.baseline_text.configure(yscrollcommand=self.baseline_text_vsb.set)
