...
BUCKET_DOWNGRADE_HOURS = {
    1: 36,    # bucket 1 -> 0 after 36 hours in yard
    2: 48,    # bucket 2 -> 1 after 48 hours in yard
    3: 216,   # bucket 3 -> 2 after 216 hours in yard
}
...
def effective_bucket(initial_bucket: int, hours_in_yard: float) -> int:
    """Downgrade bucket as dwell time in yard increases."""
    bucket = initial_bucket

    while True:
        if bucket == 3 and hours_in_yard >= BUCKET_DOWNGRADE_HOURS[3]:
            bucket = 2
        elif bucket == 2 and hours_in_yard >= BUCKET_DOWNGRADE_HOURS[2]:
            bucket = 1
        elif bucket == 1 and hours_in_yard >= BUCKET_DOWNGRADE_HOURS[1]:
            bucket = 0
        else:
            break
...
class Container:
    ...
    initial_pred_bucket: int = -1
    placement_bucket: int = -1
    service: str = ""
    vessel: str = ""
    teu: int = 1
    type_code: str = ""
    ...
    def __post_init__(self):
        if self.initial_pred_bucket < 0:
            self.initial_pred_bucket = self.pred_bucket
        if self.placement_bucket < 0:
            self.placement_bucket = self.pred_bucket

    def hours_in_yard(self, current_time: datetime) -> float:
        return max(0.0, (current_time - self.arrival_time).total_seconds() / 3600.0)

    def refresh_placement_bucket(self, current_time: datetime) -> None:
        """Update stacking priority only; does not change departure_time."""
        self.placement_bucket = effective_bucket(
            self.initial_pred_bucket,
            self.hours_in_yard(current_time),
        )
...
class Yard:
    ...
    def refresh_all_placement_buckets(self, current_time: datetime) -> None:
        for stack in self.stacks:
            for container in stack.containers:
                container.refresh_placement_bucket(current_time)

    def _below_bucket(self, stack: Stack) -> Optional[int]:
        if stack.height() == 0:
            return None
        return stack.containers[-1].placement_bucket

    def _bucket_placement_rank(self, stack: Stack, bucket: int) -> tuple:
        """
        Rank feasible stacks for bucket placement:
        1) ideal: lowest spot on top of container exactly one bucket lower
           (bucket 0 on 1, bucket 1 on 2, ...; bucket 3 on ground)
        2) fallback: lowest spot with higher priority than container below
        3) last resort: lowest legal spot
        """
        below = self._below_bucket(stack)
        height = stack.height()

        if bucket == 3 and height == 0:
            tier = 0
        elif below is not None and below == bucket + 1:
            tier = 0
        elif below is None or bucket < below:
            tier = 1
        else:
            tier = 2
    ...
    def place_bucket(self, c):
        ...
        best = min(
            feasible,
            key=lambda s: self._bucket_placement_rank(s, c.placement_bucket),
        )
...
class SimState:
    ...
    def _sim_reference_time(self) -> datetime:
        times = [c.arrival_time for c in self.containers]
        times.extend(c.arrival_time for c in self.initial_containers)

        if not times:
            return datetime(2024, 11, 6, 0, 0, 0)

        start = min(times)
        return start.replace(minute=0, second=0, microsecond=0)

    def populate_initial_yard(self):
        reference_time = self._sim_reference_time()

        for c in self.initial_containers:
            if self.strategy == "bucket":
                c.refresh_placement_bucket(reference_time)
                self.yard.refresh_all_placement_buckets(reference_time)
    ...
    def process_hour(self, current_time, next_time):
        ...
        if self.strategy == "bucket":
            self.yard.refresh_all_placement_buckets(current_time)
        ...
        for c in arrivals_now:
            if self.strategy == "bucket":
                c.refresh_placement_bucket(c.arrival_time)
                self.yard.refresh_all_placement_buckets(c.arrival_time)
                stack_id, used_overflow = self.yard.place_with_overflow(c, self.yard.place_bucket)
            else:
                stack_id, used_overflow = self.yard.place_with_overflow(c, self.yard.place_baseline)
        ...
        for c in departures_now:
            ...
            for r in reshuffled:
                if self.strategy == "bucket":
                    r.refresh_placement_bucket(c.departure_time)
                    self.yard.refresh_all_placement_buckets(c.departure_time)
    ...
    def draw_single_stack(self, canvas, stack, x, base_y, box_w, box_h):
        ...
        for tier, c in enumerate(stack.containers):
            ...
            canvas.create_rectangle(
                ...
                x + box_w,
                y_top + box_h,
                fill=BUCKET_COLORS[c.placement_bucket],
                outline=outline,
                width=width,
            )
    ...
    def update_text_panels(self):
        ...
        for c in self.containers[:10]:
            self.bucket_text.insert(
                ...
                f"pred bucket: {c.pred_bucket}, placement bucket: {c.placement_bucket}\n",
            )
